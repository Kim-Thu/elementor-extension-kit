...
namespace ElementorExtensionKit\Elements\Collection\PostList;
...
final class PostList extends Widget_Base
{
    public function get_name(): string { return 'eek-post-list'; }
    public function get_title(): string { return esc_html__('EEK Post List', 'elementor-extension-kit'); }
    public function get_icon(): string { return 'eek-brand-mark'; }
    public function get_categories(): array { return [Plugin::ELEMENT_CATEGORY]; }
    public function get_style_depends(): array { return ['eek-post-list', 'eek-pagination']; }
    ...
    protected function register_controls(): void
    {
        ...
        $this->add_control('posts_per_page', ['label' => esc_html__('Posts per page', 'elementor-extension-kit'), 'type' => Controls_Manager::NUMBER, 'min' => 1, 'max' => 50, 'default' => 10]);
    ...
    protected function render(): void
    {
        ...
        if (! ($postTypeObject instanceof WP_Post_Type) || ! $postTypeObject->public) { $postType = 'post'; }

        $allowedOrderby = ['date', 'title', 'modified', 'menu_order'];
        ...
        $args = [
            'post_type' => $postType,
            'post_status' => 'publish',
            'posts_per_page' => max(1, min(50, (int) ($settings['posts_per_page'] ?? 10))),
            'orderby' => $orderby,
            'order' => $order,
            'paged' => $paged,
        ...
        $taxonomyObject = $taxonomy !== '' ? get_taxonomy($taxonomy) : null;
        ...
        if ($taxonomyObject instanceof WP_Taxonomy && $taxonomyObject->public && $terms !== []) {
            $args['tax_query'] = [['taxonomy' => $taxonomy, 'field' => 'slug', 'terms' => $terms]];
        }

        $query = new WP_Query($args);
        if (! $query->have_posts()) {
            $emptyMessage = trim((string) ($settings['empty_message'] ?? ''));
            if ($emptyMessage !== '') { ?><p class="eek-post-list__empty"><?php echo esc_html($emptyMessage); ?></p><?php }
            return;
        }
        ?>
        <ol class="eek-post-list">
            <?php foreach ($query->posts as $post) :
                ...
                if ($excerpt === '') { $excerpt = wp_trim_words(wp_strip_all_tags((string) get_post_field('post_content', $postId)), 28); }
                ?>
                <li class="eek-post-list__item">
                    <article class="eek-post-list__article">
                        <?php if (($settings['show_image'] ?? '') === 'yes' && $imageUrl !== '') : ?><a class="eek-post-list__media" href="<?php echo esc_url($permalink); ?>"><img src="<?php echo esc_url($imageUrl); ?>" alt="" loading="lazy" decoding="async"></a><?php endif; ?>
                        <div class="eek-post-list__body">
                            <?php if (($settings['show_date'] ?? '') === 'yes') : ?><time class="eek-post-list__date" datetime="<?php echo esc_attr(get_the_date(DATE_W3C, $postId)); ?>"><?php echo esc_html(get_the_date('', $postId)); ?></time><?php endif; ?>
                            <h3 class="eek-post-list__title"><a href="<?php echo esc_url($permalink); ?>"><?php echo esc_html($title); ?></a></h3>
                            <?php if (($settings['show_excerpt'] ?? '') === 'yes' && $excerpt !== '') : ?><p class="eek-post-list__excerpt"><?php echo esc_html($excerpt); ?></p><?php endif; ?>
                        </div>
                    </article>
                </li>
            <?php endforeach; ?>
        ...
        if ($links !== []) : ?>
            <nav class="eek-pagination eek-post-list__pagination" aria-label="<?php echo esc_attr__('Post pagination', 'elementor-extension-kit'); ?>">
