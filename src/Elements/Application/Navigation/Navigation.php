...
final class Navigation extends Widget_Base
{
    ...
    protected function register_controls(): void
    {
        ...
        $this->add_control('orientation_source', [
            'label' => esc_html__('Orientation source', 'elementor-extension-kit'),
        ...
        $this->add_control('orientation_override', [
            'label' => esc_html__('Orientation', 'elementor-extension-kit'),
            'type' => Controls_Manager::SELECT,
            'default' => 'vertical',
            'options' => ['vertical' => esc_html__('Vertical / Sidebar', 'elementor-extension-kit'), 'horizontal' => esc_html__('Horizontal / Top', 'elementor-extension-kit')],
            'condition' => ['orientation_source' => 'override'],
        ]);
    ...
    protected function render(): void
    {
        ...
        if ($items === []) { return; }
        $local = ElementInstanceInheritance::local($settings, 'orientation');
        $resolved = (string) ElementDefaultResolver::resolve('app-navigation', 'orientation', $local['value'], $local['has_local'], 'vertical');
        $orientation = $resolved === 'horizontal' ? 'horizontal' : 'vertical';
        ...
        <nav class="eek-app-navigation eek-app-navigation--<?php echo esc_attr($orientation); ?>" aria-label="<?php echo esc_attr($ariaLabel); ?>">
            <ul class="eek-app-navigation__list">
                <?php foreach ($items as $index => $item) : ?>
                    ...
                    if (($item['current'] ?? '') === 'yes') { $this->add_render_attribute($key, 'aria-current', 'page'); }
                    ?>
